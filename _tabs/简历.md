...
数据中心工程师、运维开发、数据工程师、CEPH存储工程师、基础设施工程师、站点可靠性工程师
...
- 主导本地多节点多GPU超融合集群设计、落地与运维，统筹架构规划、部署实施及运维，包括网络搭建、安全配置与技术支持，支撑深度学习、数据工程等任务场景。
- 核心技术：基于Proxmox VE构建虚拟化底座，[搭配CEPH分布式存储（RBD+EC纠删码池）]({% post_url 2024-11-15-ceph %})；通过LXC/Docker实现容器化部署，FreeIPA完成统一身份认证与权限管控。
...
#### 横向扩展
- 将单进程的 CPU 和 GPU 任务 Python 代码重写为使用多进程和多线程，以优化计算负载并缩短任务执行时间。这些任务包括从文本和音频信号数据工程到深度学习模型推理的各种内容。

#### 训练词向量模型 
- 为了做研究实验，用中文语料做文本预处理和训练word2vec、fasttext、glove架构词向量模型，成果在发表在CSSCI来源期刊中。
...
- 创建和更新文档
- 计算机硬件故障排除和维修
- 穿越机全程组装和驾驶，Betaflight/ELRS/EdgeTX 设置，用 PIDtoolbox pro 调PID
- 树莓派应用项目
