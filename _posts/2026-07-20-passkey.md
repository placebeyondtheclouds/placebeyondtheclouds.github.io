...
> this is irreversible and the board will be locked to firmware maintainer's cert. in case of rp2350, the firmware will write to OTP memory page 58 on the first start. if plan to properly secure the key later, the better option is NOT to follow this section and follow **securing rp2350 (irreversible)** section instead right from the start, this way the board will be locked to our own cert. 
{: .prompt-danger }
...
the exact pico-fido2 commit is `c5aecd8eeada072e6a8ec536b7c9d28e694c9b6c` tag `v7.4.2-librekeys`
...
it will restart in application mode. reconnect it again in bootloader mode. set the permissions again. then save the current OTP state
...
**i am stuck here at enabling secure boot because apparently there is a bug in the firmware, digest length is being provisioned one byte longer that the otp**

```bash
#check
opensc-tool \
  --send-apdu 00A4040008A0583FC19B7E4F21 \
  --send-apdu 801E030000
#should say 00 00 FF 90 00 - unsecured
# 00 = secure boot disabled
# 00 = Secure Lock disabled
# FF = no recognized boot-key slot
# 90 00 = command success

#command the firmware to provision the secure boot, without secure lock first. this is irreversible, but we still have 3 more slots to write other certs
opensc-tool \
  --send-apdu 00A4040008A0583FC19B7E4F21 \
  --send-apdu 801D000000

#power cycle and check again
opensc-tool \
  --send-apdu 00A4040008A0583FC19B7E4F21 \
  --send-apdu 801E030000
#should say 01 00 00 90 00 - secure boot enabled
...
reboot, check if it works. put into bootloader mode again, set the permissions. save the OTP state:
...
enable secure lock. first `sudo apt install opensc pcscd` if not already. boot in application mode, then:
