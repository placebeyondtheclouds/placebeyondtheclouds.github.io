...
> in case of rp2350, the firmware will write to OTP memory page 58 on the first start. might program OTP memory in S3 as well. this is irreversible
{: .prompt-danger }
...
docker --context rootless run --rm \
  ...
  bash -lc '
    rm -rf build-keyhash &&
    cmake -S . -B build-keyhash -DPICO_BOARD=pico2 &&
...
turn on the board in bootloader mode, set the premissions `sudo setfacl -m "u:$(id -un):rw" "/dev/bus/usb/$(lsusb | awk '/Raspberry Pi RP2350 Boot/ { sub(/:$/, "", $4); print $2 "/" $4; exit }')"`, flash the signed firmware:
...
#command the firmware to provision the secure boot
opensc-tool \
  --send-apdu 00A4040008A0583FC19B7E4F21 \
