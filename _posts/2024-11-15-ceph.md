...
- At least 6 disk drives per pool for 4+2 erasure coding
  ...
  - solid state: same size U.2 NVMe enterprise-grade SSDs **with PLP** (enterprise-grade always are). The latter is extremely important, drives without PLP (read: consumer-grade) will power-reset themselves under heavy load, causing OSDs to fail and the entire pool to become unusable.
