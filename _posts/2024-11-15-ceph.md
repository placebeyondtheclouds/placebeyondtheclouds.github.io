...
- A cluster running `Proxmox VE 8.2` with at least 3 nodes. For this tutorial I redid the whole process using three QEMU virtual machines on my laptop, but the production hypervisors must be deployed baremetal, obviously.
...
- At least 6 disk drives per pool for 4+2 erasure coding
  ...
  - solid state: same size U.2 NVMe enterprise-grade SSDs, no consumer-grade SATA drives. ~~or at very least consumer-grade SATA **with PLP**. The latter is extremely important, drives without PLP (read: most of consumer-grade) might power-reset themselves under heavy load, causing OSDs to fail~~
