...
notes on my analog and Walksnail 5inch builds. in challenging environments where GPS can be spoofed and elrs jammed, the failsafe and gps rescue must be separated to prevent *the flyaway* (to the spoofed location). failsafe stage 2 must be set to drop, and gps rescue activated using a switch on the radio manually.
...
- ~~[MGC 2207.5 1960kv V4](/assets/images/MGC-2207.5-1960kv-V4.JPG)~~, the bearings are not very robust, the motor bells are available for sale separately 
...
- ESC [JHEMCU 65A](https://www.jhemcu.com/e_productshow/?82-JHEMCU-RUIBET-6065A-3-6S-DSHOT600-BLHELI_S-4IN1-ESC-82.html)  default J-H-15 - Bluejay, 0.19, 48kHz
- Mayatech Redbean gen4 RHCP antenna, 5.2-6GHz (lowest VSWR is at 5.7GHz), 3dBi gain, MMCX L-shape with 93mm cable.
...
- ESCs reboot right after plugging in the battery might be related to dshot600 (there is no such problem on dshot300)
...
- configure betaflight (issue `save` before leaving the CLI):
  ...
  - check [the datasheet for the magnetometer chip](https://datasheet.lcsc.com/szlcsc/QST-QMC5883L-TR_C192585.pdf) for its XYZ axis alignment, dry fit the GPS module on the drone frame with magnetometer axes pointing in the same directions as the [the default Betaflight directions](https://betaflight.com/docs/wiki/guides/current/Magnetometer), then set the actual installed orientation difference with in the CLI like
    ...
    # when located at the tail angled mount with the socket facing forward
    ...
    # when located in the front with the socket facing forward
    set mag_align_roll = 0
  ...
  - misc settings (disabling beacon here too)
  ```
  feature -AIRMODE
  beeper -ON_USB
  beacon -RX_LOST
...
- Chris Rosser's [5inch prop test results](https://drive.google.com/file/d/1DFiJmkWIDu_VJWe4ScVCCZXpYY29WDGy/view)