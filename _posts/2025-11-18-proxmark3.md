...
since the firmware must be the same version as the client, each time afer pulling a new version we must flash the bootloader and the firmware (the proxmark will reset and reconnect itself during the process):
