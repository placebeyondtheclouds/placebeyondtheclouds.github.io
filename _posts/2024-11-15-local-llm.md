...
- Ollama instance running on a local machine (or any machine on the network for that matter), accessible via open-webui running in Docker container, or Python API on the same machine or remotely.

## Prequisites

- A VM with Ubuntu 24.04 LTS
- NVIDIA GPU
